...
void ofApp::setup(){
    ...
    // setup score image
    for (int i = 0; i <= 9; i++) {
        number[i].load("img/num" + to_string(i) + ".png");
        number[i].resize(275, 370);
    }
    
    // setup background
    background_top.load("img/background_top1.jpg");
    background_bottom.load("img/background_bottom.jpg");
    
    // setup videos
    album_video.load("video/photos.mp4");
    album_video.setLoopState(OF_LOOP_NORMAL);
    album_video.play();
...
void ofApp::draw(){
    cam.begin();
    cam.setPosition(ofVec3f(2200, 620, 2500));  // Debug mode
    ...
    ///// Bottom Screen /////////////////
    background_bottom.draw(0, 0, 1080, 1920);
    
    ///// Top Screen ////////////////////
    background_top.draw(0, 1920, 1080, 1920);
    
    album_video.draw(100, 2880, 880, 590);
    
    //ofEnableAlphaBlending();
    ofSetColor(255, 255, 255, opacity);
    number[tag_counter % 10].draw(670, 1010, num_w, num_h);
    //if (tag_counter >= 10) {
        number[(tag_counter / 10) % 10].draw(405, 1010, num_w, num_h);
    //}
    //if (tag_counter >= 100) {
        number[(tag_counter / 100) % 10].draw(163, 1010, num_w, num_h);
    //}
